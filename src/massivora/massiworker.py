...
from massivora.utils import massivora_pkg_dir, setup_logging


def executor_path(module):
    pkg_dir = massivora_pkg_dir()
    return os.path.join(pkg_dir, 'executors', module)


def run_executor(module_path, argv):
    old_argv = sys.argv
    try:
        sys.argv = argv
...
def main(argv=None):
    ...
    if ns.command == 'align':
        run_executor(executor_path('align.py'), ['align.py'] + forwarded)
        return 0

    if ns.command == 'concat':
        run_executor(executor_path('concatenate.py'), ['concatenate.py'] + forwarded)
        return 0

    if ns.command == 'couple':
        run_executor(executor_path('couple.py'), ['couple.py'] + forwarded)
        return 0
